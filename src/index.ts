...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Tatoeba');
}
...
const tools: McpToolExport['tools'] = [
  ...
  {
    name: 'sentence',
    description: 'Fetch a single Tatoeba sentence record by numeric ID; returns the sentence text, language code, author, and linked audio/translation metadata.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'languages',
    description: 'List all languages supported by Tatoeba with their ISO 639-3 codes and sentence counts; use codes as `from`/`to` values in `search`.',
...
async function tatoebaGet(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw await httpError(res, 'Tatoeba');
  // Tatoeba answers 200-with-an-HTML-page when its API is down. `res.json()`
  // turned that into `Unexpected token '<', "<!DOCTYPE "... is not valid JSON`
  // — a parser complaint that named neither Tatoeba nor what happened, and read
  // like a defect on our side. parseJson checks the body before parsing and
  // says the upstream served HTML (fleet #712).
  return parseJson(res, 'Tatoeba');
}
